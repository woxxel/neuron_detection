...
from .utils import com, calculate_img_correlation, get_shift_and_flow, fun_wrapper, pickleData, gmean
...
class Sheintuch_matching:
    ...
    def __init__(self,basePath,mouse,sessions,dataSet='OnACID',SNR_thr=3.,r_thr=0.5,CNN_thr=0.6,s_corr_thr=0.3,d_thr=12,nbins=100,qtl=[0.05,0.95],use_kde=True,model='new'):
    ...
    def load_footprints(self,s,Cn_ref=None):
        ...
        pathData = os.path.join(self.para['pathMouse'],'Session%02d'%s,self.para['fp_file'])
        ...
        if os.path.exists(pathData):
            ...
            if ext=='.hdf5':
                ...
                self.session_data['SNR'] = ld['SNR_comp']
                self.session_data['C'] = ld['C']
                self.session_data['idx_eval'] = (ld['SNR_comp']>self.para['SNR_thr']) & (ld['r_values']>self.para['r_thr']) & (ld['cnn_preds']>self.para['CNN_thr'])
    ...
    def plot_registration(self,suffix='',dataSet='redetect',sv=False):
      ...
      fileName = f'cluster_stats_{dataSet}.pkl'
      ...
      if sv:
          ext = 'png'
          path = os.path.join(self.para['pathMouse'],'Figures/Sheintuch_registration_score_stats_%s_%s_%s.%s'%(self.dataSet,self.para['model'],suffix,ext))
          plt.savefig(path,format=ext,dpi=300)

    def save_model(self,suffix=''):

      pathSv = os.path.join(self.para['pathMouse'],'matching/Sheintuch_model_%s%s.pkl'%(os.path.splitext(self.para['fp_file'])[0],suffix))
